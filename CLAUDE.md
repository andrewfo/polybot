...
**Sections 0-9: COMPLETE** — Core infra, LLM, wallet, DB, market filtering, signal engine (4 providers + aggregator), Kelly criterion, order execution, web dashboard (7 tabs incl. Trades), continuous learning, monitoring/notifications, health checks, pipeline integration (3 workers). **447 tests passing**, zero TODOs/FIXMEs. Recent (Phases 1-2 of `docs/PROFITABILITY_FIX_PLAN.md`): pre-frontier edge gate (`PRE_FRONTIER_EDGE_THRESHOLD`) — free signals run first, Sonar+frontier skipped when prelim edge < 3%; `web_search` benched (`ENABLE_WEB_SEARCH_SIGNAL=false`, weight 0) and `onchain_flow` weight 0 with calibration earn-back (Brier < 0.20 over 30+ samples); `onchain_flow` rebuilt market-aware (target-distance/vol/expiry baseline, flow tilt); `record_prediction` upserts (no churn duplicates); `RESOLUTION_SIGNAL_WEIGHT` 1.3→2.5.
