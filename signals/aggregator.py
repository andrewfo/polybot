...
def _build_frontier_prompt(
    ...
    if market_type == "event":
        ...
        instructions = (
            f'Instructions:\n'
            f'1. Critically evaluate each signal source. The web_search and onchain_flow signals often produce poorly calibrated probabilities for specific price-target questions — they measure sentiment, not the probability of a specific price move. Discount them heavily when they disagree with the math model without a concrete catalyst.\n'
            f'2. Signals marked as "DIRECT RESOLUTION SOURCE" use the actual price data and calibrated volatility models. For price-target crypto markets, this is your PRIMARY signal. Your estimate should start from this model\'s probability and only deviate with specific justification (e.g., a known catalyst, broken data feed, wrong model type selected).\n'
            ...
            f'3b. For crypto markets showing both Terminal and Barrier model probabilities: evaluate which resolution type actually applies. "Will price reach X by date Y" / "Will price dip to X" is barrier (touch anytime). "Will price be above X on date Y" is terminal (price at expiry). If the selected model seems wrong for the market question, use the other model\'s probability instead.\n'
            f'4. If your final estimate differs from the DIRECT RESOLUTION SOURCE by more than 0.05, your reasoning MUST explain specifically what information justifies the deviation. Generic statements about "uncertainty" or "tail risk" are not sufficient.\n'
            f'5. Provide your final probability estimate.\n'
            f'6. Rate your overall confidence (0-1) in this estimate.\n'
            f'7. Explain your reasoning in 2-3 sentences, citing specific data points.'
        )
    ...
    return (
        ...
        f'NOTE: The market price is shown LAST deliberately. You should have already formed your estimate from the signal data above. '
        f'Do NOT adjust toward the market price. If your data-driven estimate differs from the market by more than 0.03, that is expected — it means you found an edge. '
        f'These are mid-to-low liquidity crypto markets that are frequently mispriced.\n'
        f'\n'
        f'CONFIDENCE CALIBRATION: confidence=0.8 means you believe your estimate is within 0.05 of the true probability 80% of the time. '
        f'If you are just echoing the market price or have little independent information, your confidence should be LOW (0.3-0.5), not high.\n'
        f'\n'
