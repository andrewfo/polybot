...
class ClobClientWrapper:
        ...
        while True:
            if cursor:
                result = await self._rate_limited_call(
                    self._client.get_markets, next_cursor=cursor
                )
            else:
            ...
            if isinstance(result, dict):
                ...
                all_markets.extend(markets if isinstance(markets, list) else [])
    ...
    async def get_orderbook(self, token_id: str) -> dict[str, Any]:
        """Get current order book (bids/asks with depths)."""
        result = await self._rate_limited_call(
            self._client.get_order_book, token_id
        )
        return result if isinstance(result, dict) else {"bids": [], "asks": []}

    async def get_midpoint(self, token_id: str) -> float:
        """Get midpoint price as float (implied probability)."""
        result = await self._rate_limited_call(
